...
def array_rescale_by_units(data, unit):
...
def rescale_by_units_variable(variable, units):
    if isinstance(variable, IndexVariable):
        ...
        if units is not None:
            ...
            converted = array_rescale_by_units(quantity, units)
        ...
        converted = array_rescale_by_units(variable.data, units)
...
def rescale_by_units_index(index, index_vars, units):
    ...
    for name, var in index_vars.items():
        unit = units.get(name)
        try:
            converted = rescale_by_units_variable(var, unit)
...
def rescale_by_units_dataset(obj, units):
    converted = {}
    failed = {}
    indexed_variables = obj.xindexes.variables
    for name, var in obj.variables.items():
        ...
        unit = units.get(name)
        try:
            converted[name] = rescale_by_units_variable(var, unit)
        except (ValueError, pint.errors.PintTypeError) as e:
            failed[name] = e
    ...
    for idx, idx_vars in obj.xindexes.group_by_index():
        ...
        try:
            converted_index = rescale_by_units_index(idx, idx_vars, idx_units)
...
def rescale_by_units(obj, units):
    if not isinstance(obj, (DataArray, Dataset)):
    ...
    try:
        new_obj = call_on_dataset(
            rescale_by_units_dataset, obj, name=temporary_name, units=units
        )
    except ValueError as e:
        (failed,) = e.args
...
def rescale_by_units_slice(indexer, units):
    attrs = {name: getattr(indexer, name) for name in slice_attributes}
    converted = {
        name: array_rescale_by_units(value, units) if value is not None else None
...
def convert_indexer_units(indexers, units):
    def convert(indexer, units):
        if isinstance(indexer, slice):
            return rescale_by_units_slice(indexer, units)
        elif isinstance(indexer, DataArray):
            return rescale_by_units(indexer, {None: units})
        elif isinstance(indexer, Variable):
            return rescale_by_units_variable(indexer, units)
        else:
            return array_rescale_by_units(indexer, units)
